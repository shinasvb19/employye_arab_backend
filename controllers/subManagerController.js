...
exports.createSubManager = async (req, res) => {
  try {
    ...
    const newSubManager = new subManagersSchema({
      name: req.body.name,
      employeeCode,
    ...
    await managerSchema.findByIdAndUpdate(managerId, {
      $push: { subManagers: submanagerId },
    });
...
exports.getSubManagers = async (req, res) => {
  try {
    const subManagers = await subManagersSchema.aggregate([
      {
        $lookup: {
          from: "employees",
